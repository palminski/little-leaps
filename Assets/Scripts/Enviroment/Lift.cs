...
public class Lift : MonoBehaviour
{
    ...
    {
        if (!isMoving && isActive && hitCollider.gameObject == playerTransform.gameObject && playerScript.IsGrounded() && !playerScript.IsDashing()) {
    ...
    {
        if (AudioController.Instance) AudioController.Instance.PlayLiftClose();

        yield return new WaitForSeconds(1.4f);
        if (playerScript) playerScript.RemovePlayer();
        waypointMovement.TriggerShouldMove();
        if (AudioController.Instance) AudioController.Instance.PlayLiftMove();

        yield return new WaitForSeconds(0.5f);
        LevelConnection.ActiveConnection = null;
        GameController.Instance.CollectedObjects.Clear();
        GameController.Instance.SavedSprites.Clear();
        GameController.Instance.ChangeScene(targetSceneName);
        GameController.Instance.ClearFollowingObjects();
    }
}
